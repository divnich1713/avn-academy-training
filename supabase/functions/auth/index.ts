  ...
  try {
    ...
    if (action === "instructors") {
      ...
      const instructorsRes = await client.queryObject<{
        ...
        `SELECT id, static_id, name, rank, unit, role, created_at, last_seen, discord_id, avatar_url FROM ${SCHEMA}.users WHERE (role = 'instructor' OR role = 'head_avng') AND is_whitelisted = true ORDER BY name ASC`
      );
